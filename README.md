...
The temporary `bridge` surface remains responsible only for control-relay compatibility during tlh migration work. Rich subagent result relay/ack behavior now remains `full`-only. It does not replace or provide native supervisor tooling.
...
| `bridge` | no | Keeps broker/runtime compatibility for temporary tlh migration bridging and subagent control relay behavior, but omits the local `intercom` tool, `/intercom`, `Alt+M`, reply-hint tool instructions, subagent result relay/delivery acknowledgements, and pi-intercom's duplicate compatibility `contact_supervisor` |
| `off` | no | Installs no intercom runtime or public surface |

`bridge` is intentionally narrower than `full`: it is a temporary compatibility surface for the remaining control-delivery migration work, not a replacement for native supervisor tooling (which modern `pi-subagents` already provides).
